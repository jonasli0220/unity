...
- 主列表默认纳入节点为“待提交/提交 CN release”或“待提交/提交 NA release”的需求/BUG；节点匹配兼容 `CN/NA` 与 `release` 之间有空格和无空格的写法。
- 主列表提供【包含 QA测试】复选框；每次启动默认不勾选，勾选后把节点包含 `QA测试` 的需求/BUG 纳入实时拉取和 SVN 检测范围，取消勾选后恢复只显示待提交 CN/NA release 节点。
...
- 用户点击目标 merge 后、执行 dry-run 前，必须按“单号 + 本单在该目标的全部 UI/RemoteAssets 路径”检查 release SVN log；如果目标分支已经完整提交，标记为已提交并阻止重复 merge。若工作副本还有上次失败留下的本地状态，只给出精确清理提示，不自动 revert 或覆盖。
