...
6. 列表“变更范围”会显示 `UI`、`远程资源` 或 `UI + 远程资源`。如果对应目标路径尚未配置且无法自动检测，先按弹窗选择工作副本目录。
7. 工具先对本单全部 UI/远程资源 merge group 逐 revision 执行 `svn merge --dry-run`，全部检查完成后才按 revision 顺序真实 merge 到对应工作副本。
   如果 dry-run 发现冲突，会弹窗询问是否【用 trunk 覆盖冲突】；只有你确认后，工具才会用 trunk 版本重试并 resolve 本次 merge 的文本/树冲突。
8. merge 成功后，工具会自动复制提交信息，例如：

```text
#7006319869 【外观】海外活动宣传页坐骑名称修正
```

9. 点击该行的【提交】，工具按实际变更范围打开 TortoiseSVN Commit 窗口：只改一个区域时打开一个窗口，同时改 UI 和远程资源时分别打开两个窗口。
10. 你在每个 TortoiseSVN 窗口中手动检查、粘贴相同的 message 并提交。
11. 提交完成后，回到快速 merge 窗口点击【检查提交】。
12. 工具检测到 release SVN log 已包含该单号后，按钮变为【已提交】。
13. 点击【打开单子】，在飞书/Meegle 页面里手动流转流程。
