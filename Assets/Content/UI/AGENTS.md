...
- Current development plugin name/id marker is `Unity Figma Bridge Importer v18743`. If Figma shows an older title, it is running a cached development plugin. The plugin UI must not assume `localStorage` is available because Figma may run it from a `data:` URL.
...
- Figma Paste uses a focused structured clipboard contract separate from restore/import:
  ...
  - Unity-exported Figma Components and Instances paste as references to their source Prefabs; an Instance resolves its exact main component, restores the Unity source Prefab name, and maps path-backed Figma descendant visibility to Unity active-state overrides.
...
- Keep the older post-import numbered-duplicate resolver only as a best-effort cleanup for simple Unity-created suffixes with a visible separator, such as `name 1.png` or `name_1.png`, when no exact external drag context is available. Do not treat direct trailing business numbers such as `icon_13206.png` as duplicates of `icon_13205.png`.
