...
- On replacement drop, import the image through the same prefab-local `resource` workflow, replace the selected component's Sprite, and call `SetNativeSize()`. Record the Sprite and RectTransform size as one Undo operation; the imported asset itself remains after Undo.
