...
安装 `PlayModePrefabSaver` 扩展后，Inspector 顶部的 `打开 xxx.prefab` 与 `重新加载` 之间会出现 `保存修改`。它只记录 Play Mode 中通过 Inspector 或 Scene 手柄手动调整的属性；有待同步的记录时显示为 `保存修改 (N)`，点击后把这些属性同步回源 Prefab。

保存扩展是可选的。没有安装或扩展加载失败时，不影响本工具原有的点选、拖动、打开 Prefab 和重新加载功能。
...
- Inspector 的打开、可选保存、重新加载按钮只在 Play Mode、且当前选中的是运行时 UI 对象时显示。

## 范围

当前版本面向 Unity UGUI（`Graphic`、`Image`、TMP UI 文本等）。没有可见 `Graphic` 的空节点，以及没有 Collider 的 3D/2D 场景物体，不属于本工具的拾取范围。

工具是纯 Editor 功能；点选、临时拖动、打开和重新加载本身不会写入场景或 Prefab。只有用户明确点击可选扩展提供的 `保存修改` 时，扩展才会备份并写入源 Prefab。
