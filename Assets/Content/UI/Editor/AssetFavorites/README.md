...
- 已收藏过的 Hierarchy / Prefab Stage 节点再次拖入收藏夹，或再次使用 `★ 收藏为节点模板` 时，会询问 `替换原收藏` / `新增一份` / `取消`。选择替换会保留原收藏卡片和所在目录，只更新节点模板内容、尺寸、组件信息与预览；选择新增会创建一张新的收藏卡片。
...
- 节点优先按模板内容哈希去重；没有可用内容哈希时才按来源节点 ID 兜底。也就是说，同一个来源节点导出的不同样式变体会作为不同收藏导入；新增节点会生成新的本地模板 GUID，不覆盖双方原文件。
