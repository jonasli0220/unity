...
- Opening a Prefab Stage must immediately restore Prefab Hierarchy content, even when the Inspector button that opened it retains keyboard focus and no Scene-view focus event occurs.
...
- Verify opening a runtime UI Prefab from the Inspector while the Hierarchy shows runtime scenes immediately reveals the opened Prefab tree.
